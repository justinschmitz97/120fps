...
- Windows cleanup hazard: `git worktree remove --force` recurses through a junction instead of unlinking it, so removing the worktree while `linkNodeModules`'s links are still in place deletes files out of `repoRoot`'s real, shared `node_modules` rather than just detaching the pointer. `unlinkNodeModules` (`src/compare.ts:144`) detaches every link `linkNodeModules` made before either of `compareAgainstRef`'s cleanup paths runs, so neither ever walks through one. `test/unit/reference-worktree-linking.test.ts` covers the invariant against a real `git worktree remove --force`, not a plain `fs.rmSync`, because Node's own recursive delete already treats a reparse point as a leaf and would not have caught the regression.
