...
### M132: every warning on a typical run is true, printed once, and names what to do: in progress

- **About:** of the warnings a passing run prints, five are wrong, repeated, or not the tool's to print: a component with no props is hedged as maybe a failed extraction, an honored Vite alias or a named plugin list is called ignored or anonymous, a project-level note prints three times in one dry run, Node's own `MODULE_TYPELESS_PACKAGE_JSON` warning about the project's own config file reaches the terminal, and `--no-css` is a no-op in the dry run.
...
### M133: a render failure names its provider, its bad value, and nothing that never rendered: in progress

- **About:** three of six run-7 render failures printed a generic remedy although the page error and the component's own first import already named the missing provider; a growth hint was fitted over combos that never rendered; and a string-typed dimension prop synthesized the placeholder `"test"`, rendering a malformed SVG on every sample, with the disclosure that would have named it gated off.
- **Why this way:** the provider-scope table covers `@mantine/` and `@chakra-ui/` beside `@radix-ui/`, and candidates are ranked by whether the captured page error names them; no hint is derived from a combo whose `renderHealth` is `error`; a dimension-named string prop synthesizes `"16"` with provenance `heuristic`; a synthesized placeholder that still reaches the DOM is disclosed as `[harness fault]` without changing the verdict.
...
### M136: a Vue app's generated auto-import maps resolve its components and composables: in progress
