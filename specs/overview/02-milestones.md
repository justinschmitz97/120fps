...
One entry per milestone in `specs/milestones/`: what it addressed, why the approach was chosen, and what it achieved. Distilled 2026-08-18 from the milestone spec files, which remain the source of truth. Status values are reproduced verbatim from each spec's frontmatter — the vocabulary drifts across eras (`done` → `approved` → `implemented`).
