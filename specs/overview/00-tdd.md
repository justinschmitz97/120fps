...
## Modules
Nine stage directories under `src/`, in dependency order, plus the curated `src/index.ts` surface. Each stage's own `index.ts` is the cross-directory import point for that stage and is not listed as a row below.

### `shared`
Dependency-free helpers, one per fact, imported by every other stage.

| file | role |
|---|---|
| `clone.ts` | `cloneDeep` deep-clones a value so combo/preset generation never shares identity across combos. |
| `fs.ts` | Path and manifest primitives: `toPosix`/`resolvePosix`, `pathKey`, `isFile`/`isDirectory`, `readJsonFile`. |
| `git.ts` | `findGitRoot` walks upward for a `.git`-holding ancestor, used by the gitignore advisory and worktree pruning. |
| `regex.ts` | `escapeRegex` turns a literal string into a safe regex fragment. |
| `run-state.ts` | Process-lifetime import-cycle-reported flag (`setImportCycleReported`, `wasImportCycleReported`) so a runtime cycle warns once per run. |
| `stats.ts` | `computeMedian`, `computeP95` (type-7 interpolated quantile), `computeCV`/`computeCvPercent` (sample stddev, n-1): the statistics primitives every measurement and report layer shares. |

### `project`
Resolves what the project is and how its module graph and toolchain behave, before any harness exists.

| file | role |
|---|---|
| `compiler-options.ts` | `projectCompilerOptions`/`createCompilerOptions` build the `ts.CompilerOptions` prop extraction and preflight both parse against, resolved via `findCompilerConfig`. |
| `framework.ts` | `detectFramework` resolves react/vue/vanilla from the member manifest, then the workspace manifest, then an install probe, failing closed to `vanilla`; `FRAMEWORK_MANIFEST_UNREADABLE`, `SOLID_AND_REACT_DECLARED`, `PREACT_UNSUPPORTED_WARNING`. |
| `model.ts` | The two-level project model: `resolveProjectModel`/`findWorkspaceRoot` (`memberRoot` plus install-governing `workspaceRoot`, first `pnpm-workspace.yaml`/`workspaces`-field manifest/lockfile wins, bounded at a `.git`-holding ancestor), `declaredPackages`/`isPackageDeclared`/`isPackageAvailable` (declaration, or an ancestor `node_modules` walk from `memberRoot` to the filesystem root mirroring CommonJS resolution; `workspaceLevels` stays, used by `fsAllowDirs`), `findCompilerConfig`, `detectPnP`, `findProjectRoot`, `readProjectManifest`. |
| `preflight.ts` | `runPreflight` walks the import graph for server-only/`use server`/async-component/Node-builtin hits and provider-hook detection (`PROVIDER_LIBRARIES`, `detectProviderImport`, `detectLocalProviderModule`, `providerCandidateLabels`) before any harness directory or dev server exists. |
| `preflight-gates.ts` | Hit classification and remediation text: `TRANSFORM_RECOGNIZERS`, `recognizeTransform`, `HARD_REMEDY`/`preflightFailureMessage`, `NODE_BUILTIN_WARNING`, `PROJECT_TRANSFORM_WARNING`, `PREFLIGHT_BYPASSED_WARNING`, `PreflightHardRejectionError`. |
| `react-compiler.ts` | React Compiler detection and resolution: `detectReactCompiler`, `resolveReactCompiler`, `resolveReactCompilerState`, `loadReactCompilerPlugin`, `reactCompilerRuntimeDeps`; declaration-only across both roots, because the compiler rewrites measured code and a hoisted transitive copy is not a statement the project ships it. |
| `resolve.ts` | Module resolution mechanics: `resolveLocalImport`, `resolveTarget`/`resolveDirectoryEntry`, `exportsRootTargets`/`exportConditionTargets` (package.json `exports` map), `resolveSubpathImport`. |
| `transforms.ts` | Loadable project-transform plugins: `SUPPORTED_TRANSFORM_PLUGINS`, `resolvePluginFactory`, `detectProjectTransforms`, `stripServerHooks`, `loadProjectTransformPlugins`. |
| `tsconfig-aliases.ts` | `loadTsconfigAliases` builds Vite resolve aliases from tsconfig/jsconfig `paths`; `ALIAS_SHAPE_WARNING`/`WORKSPACE_ROOT_ALIAS_WARNING`/`TYPES_ONLY_ALIAS_WARNING` disclose a wildcard-shape mismatch or a types-only target. |
| `vue-sfc.ts` | The project's own SFC parser, resolved and cached via `createRequire`: `loadVueCompiler`, `parseSfcScript`, `isVueFile`, `virtualScriptPath`, `VUE_COMPILER_MISSING`. |

### `props`
TS Compiler API prop extraction bound to the resolved target component, and every value/combo/matrix/delta generator downstream of a `PropSchema`.

| file | role |
|---|---|
| `candidates.ts` | `findComponentPropsType` collects every top-level component declaration as a candidate; `selectMeasuredExport` picks the default export (following `memo`/`forwardRef`/alias chains) then a file-stem match via `normalizeComponentName` then the first exported, with a self-consistency guard against a wrong follow; `componentStem`. |
| `classify.ts` | `typeToSchema` classifies a TS type into a `PropSchema`; `warnDegenerateProps`/`warnRecursiveType`, `MAX_TUPLE_ARITY` (8), `isBooleanUnion`. |
| `composition.ts` | `inferComposition` (prefix grouping plus suffix taxonomy) infers a `CompositionTree`; `shouldRollbackComposition`/`COMPOSITION_EMPTY_WARNING` cover the trial-mount rollback; `buildFixtureScaffold`/`buildUncomposedFixtureScaffold`. |
| `delta-pairs.ts` | `generateDeltaPairs` builds pairwise prop deltas capped at `MAX_DELTA_PAIRS` (128); `countDeltaPairSpace` reports the uncapped total. |
| `exports.ts` | `scanExports`/`extractExports`/`extractAllProps` (AST export walker), `followReExportedComponent` (`RE_EXPORT_HOPS` 4), `projectSourceFiles` (the M39 fingerprint's file set). |
| `extract.ts` | `extractProps`/`extractPropsDetailed` (the latter takes an `onWarning` sink, backing `--explain-props`), `detectScalingProps`, `UNTYPED_JS_COMPONENT_WARNING`. |
| `presets.ts` | `<stem>.props.tsx` or `.ts` detection and loading: `detectPropPresets`, `loadPropPresets`, `applyPropPresets`, `PresetRef` (AST-literal transport for non-literal values), `UNKNOWN_PRESET_PROPS_WARNING`. |
| `program.ts` | `createCachedProgram`, the process-lifetime parsed-program cache; `resetExtractionCache`/`extractionCacheStats` test hooks. |
| `rank.ts` | `propRank` orders extracted props by relevance; `MAX_PROPS` (32) caps the schema. |
| `schema.ts` | The `PropSchema`/`ExportInfo`/`ScalingPropMatch`/`PropWarningRecord` type contracts shared by every extractor and generator in this directory. |
| `serialize.ts` | `serializeProps`/`FUNCTION_MARKER`, the one place a prop combination crosses into the page's serializer. |
| `synthesize.ts` | `synthesizeValue`/`synthesizeElement` (array/object/tuple value synthesis, depth- and cycle-capped via `PROP_SYNTH_MAX_DEPTH` 4), degenerate-marking (`warnSynthesizedRequiredObject`, `SYNTHESIZED_REQUIRED_OBJECT_WARNING`). |
| `values.ts` | `generateCombinations` (stratified sampling, cap 64, de-duped before any cap), `countCombinationSpace`, `generateScalingCombos`, `fillArray`, `generatePropMatrix`/`pairwiseCover`/`shouldAutoActivateMatrix`, `selectMatrixCombos` (anchor cell first then ascending Hamming distance), `selectRepresentativeCombos`. |
| `vue.ts` | Vue `defineProps`/`withDefaults` extraction over a virtual-script program: `findDefineProps`, `applyWithDefaults`, `VirtualScripts`, `extractVueProps`. |

### `harness`
Builds and serves the Vite harness page the browser measures against; the project's own `vite.config` is never loaded.

| file | role |
|---|---|
| `build.ts` | `buildAndServe` boots the dev server with `configFile: false` (the project's own Vite config is never loaded) and `server.watch: null` (nothing edits files mid-run; the watcher's initial scan otherwise stalls the first navigation by about 9s); `HarnessResult`, `VITE_START_FAILED`, `SWEEP_DEP_WARNING`. |
| `bundler-failure.ts` | Diagnoses a Vite build failure after the fact: `presentBundlerFailure`, `diagnoseUnbuiltWorkspacePackage`, `detectPackageManager`, `packageManagerRunCommand`, `findLikelyGenerateCommand`. |
| `css.ts` | `discoverGlobalCss`'s entry-import-graph layer: `findProjectEntry`, `rankedStylesheets`/`largestStylesheet` (bounded fallback), `packageStylesheetCandidates`. |
| `deps-scan.ts` | `scanExternalDeps` recursively follows imports (including tsconfig and shim aliases) to pre-populate `optimizeDeps.include`; `BROKEN_ALIAS_WARNING`, `TYPE_ONLY_PACKAGE_WARNING`. |
| `dirs.ts` | Harness-dir lifecycle: `createHarnessDir` (writability guard, `HARNESS_DIR_UNWRITABLE`), `sweepStaleHarnessDirs`/`sweepStaleTmpDirs` (both run once per `createServerPool()`), `removeHarnessDirWithRetries`. |
| `entry.ts` | Entry-file generation: `generateEntry`/`generateComposedEntry`/`generateVueEntry`, `renderTreeHelper`/`vueRenderTreeHelper`, `setupBlock`/`setupApiBlock` (wrapper `setup`/`teardown`), `presetImportLine`/`presetResolverBlock`. |
| `env.ts` | `readEnvDefines`/`parseEnvFile` expose only `NEXT_PUBLIC_*`/`VITE_*` keys as browser `process.env` defines (`ENV_DEFINE_PREFIXES`). |
| `exports.ts` | `detectComponentExport` (default export then stem match via `normalizeComponentName` then first exported, or an explicit `#ExportName` target), `detectWrapper`/`resolveWrapper`, `sfcProducesComponent`/`SFC_NO_COMPONENT`, `vueComponentName`, `detectScaleExport`. |
| `prebuild.ts` | `collectStaticPreBuildWarnings` gathers warnings knowable before the dev server boots. |
| `renderer.ts` | `rendererFor` keys the renderer adapter on the measured file's extension; `assertReactDomClient`/`REACT_DOM_CLIENT_MISSING` (React 18 boot gate); `isOutsideRoot`/`componentImportPath` (cross-drive `/@fs/` routing). |
| `server.ts` | `createServerPool` (keyed by projectRoot/cssFiles/wrapPath/compiler-active/noShims), `fsAllowDirs` (dev-server file-access allowlist, cross-drive `extraDirs`), `unionCachedDeps` (dep-cache-stable optimize list). |
| `shims.ts` | `SHIM_MODULES` (10 entries) tagged `isShim: true`, `buildShimAliases`, `unshimmedNextModules`/`UNSUPPORTED_NEXT_MODULE_WARNING`. |
| `shims/` | Ten Next.js module shims (`next-image`, `next-link`, `next-navigation`, `next-dynamic`, `next-script`, `next-head`, `next-router`, `next-font-local`, `next-headers`, `next-video-player`): browser-compatible profiling stand-ins, not polyfills. |
| `stylesheets.ts` | `entryStylesheetImports`/`detectGlobalCss` (`GLOBAL_CSS_CANDIDATES`), `validateCssFiles`, warnings `CSS_IMPORT_SKIPPED_WARNING`/`CSS_PREPROCESSOR_MISSING_WARNING`/`CSS_FALLBACK_WARNING`/`CSS_DROPPED_WARNING`. |
| `style-tooling.ts` | `resolveStyleTooling` (decoupled from whether a stylesheet was found): `detectTailwindVite`/`loadTailwindVitePlugin`, `detectUnsupportedStyleEngines`/`UNSUPPORTED_STYLE_ENGINE_WARNING`, `findPostcssConfigAbove`, `cssImportSpecifier`/`cssImportBlock`. |
| `vite-config.ts` | `readViteConfigData` reads `vite.config.*` as text via `ts.createSourceFile`, never imported or executed: recovers a literal `publicDir` and `resolve.alias`; `VITE_CONFIG_IGNORED_WARNING` names anything it could not fold. |
| `workspace-entries.ts` | `resolveWorkspaceSourceEntry`/`workspaceSubpathSourceEntries` resolve an unbuilt workspace package to its source entry. |

### `browser`
Playwright session and measurement primitives shared by every measurement pass.

| file | role |
|---|---|
| `discovery.ts` | `discoverInteractions` DOM-walks for interactive elements (`InteractionDescriptor`, `toDescriptor`), covering `#root` and portal body children. |
| `dom.ts` | `detectAnimations`, `countComponentNodes`/`totalComponentNodes`, `classifyMeasuredState`/`installMeasuredStateProbe`/`probeLateMutation` (`MEASURED_STATE_HOLD_MS` 120), `collectUnresolvedSpriteRefs`. |
| `measure.ts` | `measureMount`/`measureRerender`/`runMountUnmount`, `measureWrapperOverhead`, `applyWrapperViewport`, `rafFence`, `warmupsForPosition`, page actions `mountAndWait`/`mountAndTrace`/`rerenderAndTrace`. |
| `noise.ts` | `probeMachineNoise`/`classifyNoise` (`NOISE_PROBE_SAMPLES` 7, `NOISE_CV_PERCENT` 15, `HOSTILE_CV_PERCENT` 30), `buildNoiseReport`, `NOISY_RUN_WARNING`/`HOSTILE_RUN_WARNING`. |
| `observers.ts` | Opt-in Event Timing/LoAF/layout-instability acquisition: `installObservers`, `beginObservedWindow`/`readObservedWindow`, `observedInteractionMs`. |
| `pacing.ts` | `createFramePump`/`MEASUREMENT_BROWSER_ARGS` (begin-frame-driven pacing for lifecycle sessions), `createBrowserPool`, `FRAME_PUMP_WARNING`. |
| `page-errors.ts` | `attachPageErrorCapture` (`PageErrorCapture`/`PageErrorDrain`; `pageerror`, console, `requestfailed`, response 400 and above), `gotoWithErrorContext`/`enrichTimeoutError`, `enrichPhaseError`. |
| `portal-probe.ts` | `probePortals` exercises `aria-haspopup` triggers and walks new body children for portal content. |
| `retry.ts` | `withContextRetry`/`RetryBudget` (`DEFAULT_RETRY_BUDGET` 2, `RETRY_BUDGET_EXHAUSTED_NOTE`), `withFrameStarvationRetry`, `isContextLostError`. |
| `session.ts` | `enterHarness`/`runHarnessSession` (the shared session preamble), `openMeasurementSession`, `CdpHolder`/`refreshCdpSession`, `suspendThrottle`, `tryCollectGarbage`. |
| `settle.ts` | `settleStyles` returns `FontSettleResult { settled, failedFamilies }` (every `document.fonts` entry whose `status === "error"`); `reportFontSettle` raises `FONT_SETTLE_WARNING`/`FONT_LOAD_FAILED_WARNING`; `needsStyleSettle`. |
| `trace.ts` | `collectTrace`/`parseTraceDuration` (`TraceEvent`), `buildTimingResult`, the `MountResult`/`RerenderResult` shapes. |

### `report`
The Report/Baseline type contracts and every pure function that turns measured samples into a verdict or a printed line.

| file | role |
|---|---|
| `baseline-io.ts` | `loadBaseline`/`saveBaseline`, `computeEnvKey`/`baselineKey` (composite `<component>#<envKey>` slots), `selectBaselineEntry` (cross-environment fallback), `BASELINE_SLOT_TTL_DAYS` (90). |
| `budget.ts` | `loadBudgetConfig`/`validateBudgetConfig`, `compareBaseline`, `buildEnvFingerprint`/`classifyEnv`/`describeEnvDiff`/`envAdvisory`, `computeSourceFingerprint`, `METRICS_REVISION` (4). |
| `ci.ts` | Pure `Report` serializers: `formatMarkdown` (`--report-md`), `formatJUnit` (`--report-junit`); no measurement state, filesystem, or network. |
| `hints.ts` | `HINTS`/`HintId` (finding-to-remediation mapping), `hintsForReport`, `formatHints`, `MEASUREMENT_BASIS_LINE`. |
| `metrics.ts` | `parseMetrics`/`computeINP`/`createCalibrationTrace`, `attributeCost` (`buckets[].durationMs` is the mean per window; attributes from the last `node_modules/` segment so a pnpm store path attributes to the real package). |
| `phases.ts` | `createPhaseClock`/`PhaseTimings`, `formatPhaseBreakdown`, the pipeline-phase-boundary bookkeeping behind the progress heartbeat. |
| `stats.ts` | `classifyTier` (portal/animation is a floor of T3, `max(sizeTier, T3)`), `computeVerdict`, `evaluateCurve`/`computeCurveVerdict`, `isSuperlinearGrowth`, `buildCurveReport`/`buildMatrixReport`, `deriveReportMode`/`describeMode`. |
| `terminal.ts` | `attachWrapperReport`, `formatBaselineSection`, `appendHints`/`appendWarnings`/`appendPageErrors`, `formatNoiseLine`. |
| `terminal-modes.ts` | `formatTable` (the terminal report for every mode), `formatCurveViolation`, `CURVE_NOT_ACTIVATED_WARNING`. |
| `types.ts` | The `Report`/`ComboReport`/`EnvFingerprint`/`ScalingCurveReport`/`MatrixReport`/`BaselineComparison` type contracts, `DEFAULT_THRESHOLDS`, `TIER_BUDGETS`, `CHURN_DEGRADATION_LIMIT`, `LEAK_BYTES_PER_CYCLE`. |

### `analysis`
The higher-order measurement passes built on `browser`, `props` and `harness`.

| file | role |
|---|---|
| `compare.ts` | `compareAgainstRef` (`--compare`, interleaved A/B via a git worktree), `linkNodeModules`/`unlinkNodeModules`, `pruneStaleWorktrees`, `distinguishable` (sample-range comparison). |
| `exploration-loop.ts` | `exploreCombo`, the per-combo mount-trace-interact-deepen loop that `explore` drives. |
| `explorer.ts` | `explore` (state graph builder), `selectExploreCombos` (run-level budget: `DEFAULT_MAX_COMBOS` 8, `DEFAULT_TOTAL_WALL_CLOCK_MS` 300000), `probeVolatileRegions`/`computeDomHash`, `EXPLORE_BUDGET_WARNING`. |
| `isolation.ts` | `runIsolationPhases` (`measureChurn`/`measureMemory`/`measureStrictMode`), `parseIsolationPhases`, `computeIsolationVerdict`, `strictModeUnsupported`/`VUE_STRICTMODE_ERROR`. |
| `react-probe-entry.ts` | `generateProbeEntry`/`generateProbeHtml`, the isolated React-analysis harness page. |
| `react-profiler.ts` | `runReactAnalysis`, `resolveReactDomIdentity`/`isSupportedReactDomVersion` (React 16.5-19.x gate before opening a browser), `diffSnapshots`/`detectMemoBailouts`/`detectContextFanOut`, `computeCallbackIdentityDelta`, `computePortalOrphans`. |
| `stress-patterns.ts` | `resolveStressPattern`/`executeStressPattern` (pointer-drag, keyboard-sweep, hover-sweep, open-close-10, multi-keystroke, rapid-toggle-11, scroll-sweep, single-shot), `findAriaGroupSiblings`, `countPatternEvents`. |

### `pipeline`
Orchestration. `analyze()` ties prop extraction, harness build and every measurement mode together; `buildReport` assembles the `Report`; `pipeline/modes/` holds the per-mode runners.

| file | role |
|---|---|
| `analyze.ts` | `analyze` (full pipeline orchestrator), `AnalyzeOptions`, `writeReportJson`, `resolveProgressReporter`. |
| `build-report.ts` | `buildReport` (combo path), `applyBaselineWorkflow`/`buildBaselineEntry` (`--check`/`--save-baseline`, shared by the combo and isolation paths), `buildCssReport`/`buildReactCompilerReport`. |
| `estimate.ts` | `estimateRunCost`/`estimateExplainedRunCost`, the wall-clock estimate behind the progress heartbeat's phase budgets. |
| `explain-props.ts` | `explainProps`/`formatExplainProps` (`--explain-props`): resolved component, `file:line`, every extracted prop, degenerate reasons, curve/matrix eligibility; no Vite, no browser. |
| `fixtures.ts` | `detectFixture`/`isFixturePath`/`hasScaleExport`, `trialMountComposition`, `writeFixtureScaffold` (`--init-fixture`). |
| `modes/combo.ts` | `runComboMode`: the standard combo pipeline, `boundScalePointsByProbeCost` (`SCALE_PROBE_GATE_MS`), `STRATIFIED_SAMPLE_WARNING`, `COMBO_CAP_WARNING`. |
| `modes/context.ts` | `ModeContext`, `predictMode`, `computeEffectiveSamples`/`EFFECTIVE_SAMPLES_WARNING`, `detectComponentName`, `animatedIndices`, `collectReactOptimizations`. |
| `modes/curve.ts` | `runCurveMode`/`resolveCurveMatch`, `applyAutoScalingCurves`, `CURVE_ALL_POINTS_EMPTY_WARNING`. |
| `modes/isolation.ts` | `runIsolationMode`, `LEAK_VERDICT_NOISE_QUALIFIED_WARNING`. |
| `modes/matrix.ts` | `runMatrixMode`, `selectMatrixCombos`-driven cap (`MATRIX_CELL_CAP_WARNING`), `MATRIX_AUTO_ACTIVATED_NOTICE`, `propDeltasFromMeasured`/`measureStandardPropDeltas`. |
| `phases.ts` | `prepareRunInputs`, `createSchemaExtractor`/`createSourceFingerprint`/`createHarnessContextAttacher`, `runPreflightPhase`, `classifyHarnessFault`, `rollbackEmptyComposition`. |
| `remedies.ts` | Failure-message remediation text: `ZERO_PROPS_WARNING`, `RE_EXPORT_MEASURED_DISCLOSURE`, `alternativeExportNote`, `renderFailed`. |
| `resolve.ts` | `resolveCssFiles`, `resolveWrapPath`, `resolveFramework`, `resolveProjectPaths`. |
| `verdict-reuse.ts` | `optionsAllowVerdictReuse`/`tryReuseStoredVerdict` (the M39 fingerprint-reuse gate), `projectConfigFingerprintFiles`, `collectMachineInfo`. |
...
| file | role |
|---|---|
| `args.ts` | `parseArgs`/`CliArgs`, `KNOWN_FLAGS`, `resolveCurveOption`/`resolveMatrixOption`/`resolveIsolationOption` (disable is `false`, absent is `undefined`). |
| `errors.ts` | `formatCliError`, `nodeVersionError`/`MIN_NODE_MAJOR` (22, checked in `main()` before `parseArgs`, exit 2), `resolveFatalProcessError`. |
| `gitignore.ts` | `gitignoreCoversFile`/`needsGitignoreAdvisory`/`gitignoreTipPatterns` (`findGitRoot` lives in `shared/git.ts`), `GITIGNORE_SUGGESTED_PATTERNS`. |
| `help.ts` | `helpText`/`printHelp`. |
| `lifecycle.ts` | `registerTerminationHandlers`/`abortRun` (`TERMINATION_SIGNALS`), `armExitWatchdog`, `closePoolsBounded`, `sweepHarnessDirsAfterClose`. |
| `main.ts` | The module-private `main()` entry (shebang, wires args to analyze to report write, not exported), plus `formatWallClock`, `resolvedRootsOutput`. |
| `paths.ts` | `expandComponentPaths` (files/directories/globs, tested against the walked path normalized relative to `process.cwd()`), `resolveReportPaths` (case-folded collision key so `Card.tsx` and `card.tsx` reports never overwrite each other), `isComponentFile`. |
...
| file | role |
|---|---|
| `index.ts` | Re-exports the twelve runtime entry points and the option/config/Report types; see the paragraph below for the full list. |

Value imports point downward: cli to pipeline to analysis to report to browser to harness to {props, project} to shared; `report` may import `browser`, `props` and `project` but not `analysis` or `harness`. Type-only imports are free in every direction except into `cli` and `pipeline`. A cross-directory value import goes through the target stage's `index.ts`, which is that stage's one cross-directory import point; an import between files inside the same stage (including `harness/shims/` and `pipeline/modes/`) goes straight to the file. `test/unit/module-boundaries.test.ts` parses every import in `src/` and enforces this order. `test/unit/module-ratchets.test.ts` enforces the 800-line cap per file, zero milestone tokens (`M68`, `M72`, and similar) or `used to`/`previously`/`no longer` in comments, and unique top-level function names across `src/`. `src/index.ts` exports the curated public surface: the twelve runtime entry points `analyze`, `buildReport`, `DEFAULT_THRESHOLDS`, `TIER_BUDGETS`, `formatMarkdown`, `formatJUnit`, `loadBudgetConfig`, `validateBudgetConfig`, `hintsForReport`, `formatHints`, `HINTS`, `parseArgs`, plus the Report types and `AnalyzeOptions`, `BuildReportInput`, `BudgetConfig`, `BaselineMetrics`, `BaselineEntry`, `HintId`, `CliArgs`, `PropSchema`, `PropCombination`. Tests import stage indexes or files directly; they are not consumers of the package surface.
...
## Tests
Current suite: 329 unit test files, 4805 unit tests (4803 passing, 1 skipped, 1 pre-existing failure `test/unit/vue-setup-inject-evidence.test.ts` that depends on vitest's hoisted `NODE_PATH`), 62 e2e files (vitest; e2e drives real Chromium). Per-milestone "N new tests" notes below are historical; this line is the source of truth. Repo CI (`.github/workflows/ci.yml`, Node 22.x/24.x/26.x) runs `pnpm build` (tsc: type-checks and emits the `dist/shims` the nextjs-shim unit tests read) and the unit suite only: e2e is excluded there for the flakiness documented below. `pnpm test` and `pnpm test:unit` both run `vitest run test/unit/` (matching CI); `pnpm test:e2e` runs `vitest run test/e2e/`; `pnpm test:all` runs the full `vitest run`.
...
- The cheapest requested point is measured inside the run's own mount batch, immediately after the prop combos and at the batch's sample count, and the gate reads that measurement mid-batch (M134 C2): over `SCALE_PROBE_GATE_MS` (80ms, T4's mount budget) the batch stops there, only that point is kept, and `SCALE_PROBE_COST_WARNING` states what was skipped and why: bounding the cost behind a 46.9s single-combo dogfood reproduction. It is therefore measured once, warm, at the batch's per-combo warm-up count, never alone at 3 samples.
...
### M74: environment advisories and font-load diagnostics (done)
**Goal**: Three portability-audit findings go from silent to diagnosed: a plain-Preact project resolves to `vanilla` with no signal React-family analysis was skipped, a 404'd or decode-failed webfont settles silently with no signal fallback-font metrics were measured, and the tool writes report/baseline JSON into the user's repo with no gitignore awareness.

**Builds on M68/M72** (framework detection, declared-vs-available primitives), **M70** (font-settle warning plumbing).

**Scope**:
- `detectFramework` (`src/react-profiler.ts`) gains a tail check: `resolved === "vanilla"` and `isPackageDeclared("preact", memberRoot, workspaceRoot)` calls `onWarning` with `PREACT_UNSUPPORTED_WARNING(memberRoot)`, module-level export only. Framework resolution is unchanged; the project still measures.
- `settleStyles` (`src/measure.ts`) returns `FontSettleResult { settled: boolean; failedFamilies: string[] }` instead of a bare boolean, collecting the deduplicated `family` of every `document.fonts` entry with `status === "error"` inside the same `page.evaluate` call. `reportFontSettle(result: FontSettleResult, onWarning?)` raises `FONT_SETTLE_WARNING` when not settled and, separately, `FONT_LOAD_FAILED_WARNING(failedFamilies)` when any family failed — both can fire on one call. `src/analyze.ts`'s standalone `fontsSettled` boolean pattern is replaced with the same `reportFontSettle` call, so both warnings reach `report.warnings` through the one shared seam.
- New `src/cli.ts` functions: `findGitRoot(startDir)`, `gitignoreCoversFile(content, filename)` (literal or single-wildcard match, not a full glob engine), `needsGitignoreAdvisory(gitRoot, writtenFilenames)`. `main()` prints `GITIGNORE_ADVISORY_HINT` once, after the measurement loop, when `!args.ci` and a written report/baseline filename is uncovered; never writes to `.gitignore`.
- See `specs/milestones/m74-environment-advisories.md`. 30 new tests.

**Does NOT include**: rejecting or gating a Preact project, retrying a failed font load, a full gitignore pattern engine, writing to the user's `.gitignore`.

### M75: import diagnosis and availability coverage (done)
**Goal**: Three annoyance-grade residuals from the M67-M73 audit: an unrecognized import (wasm, shader) fails inside Vite naming only the importing file instead of the missing plugin; `isPackageAvailable`'s per-level probe stopped at `workspaceRoot`, reporting a package unavailable that every `createRequire`-based loader in the codebase can actually resolve; and three repo shapes (Solid, Preact, JS+jsconfig) existed only as temp trees built inside test bodies.

**Builds on M69** (`TRANSFORM_RECOGNIZERS`), **M68** (`isPackageAvailable`/`workspaceLevels`), **M72** (Solid hard rejection).

**Scope**:
- `TRANSFORM_RECOGNIZERS` (`src/preflight.ts`) gains `wasm` (bare `.wasm`, owner `vite-plugin-wasm`; `?init`/`?url` excluded as Vite core's own) and `shader` (`.glsl`/`.wgsl`/`.vert`/`.frag`/`.geom`/`.comp`, owner "a shader loader plugin"). Both flow through the existing non-fatal `transforms` bucket unchanged.
- `isPackageAvailable` (`src/project-model.ts`) becomes `isPackageDeclared(…) || isInstalledOnResolutionChain(memberRoot, pkg)`: a new private probe walking every ancestor of `memberRoot` to the filesystem root (skipping a `node_modules` directory as a base, mirroring Node's `_nodeModulePaths`), replacing the per-level `workspaceLevels` probe that stopped at `workspaceRoot`. Deliberately still a directory probe, not `require.resolve`: pnpm's test-time `NODE_PATH` makes `require.resolve` see every package from every directory, and a package's `exports` map can make `require.resolve("<pkg>/package.json")` false-negative where `require.resolve("<pkg>")` succeeds. `isPackageDeclared` does not gain the fallback — it still gates the React Compiler contract and the Solid hard rejection, where availability is not evidence of what the project ships. `workspaceLevels` stays, used by `fsAllowDirs`.
- Three real fixture directories (no `node_modules`, installs git-ignored): `fixtures/solid-project/` and `fixtures/preact-project/` each carry their own `pnpm-lock.yaml` so `findWorkspaceRoot` stops at the fixture instead of climbing to this repo's own React-declaring root; `fixtures/jsconfig-project/` needs no lockfile since config discovery takes the nearest hit. Tests exercise each through the real functions: `runPreflight` for the Solid hard rejection and Preact passing that gate, `findCompilerConfig`/`loadTsconfigAliases` for the jsconfig tree.
- See `specs/milestones/m75-import-diagnosis-and-availability.md`. 26 new tests.

**Does NOT include**: loading the newly recognized wasm/shader plugins (diagnosed, never compiled), a generic unknown-extension recognizer, `.wasm?init`/`.wasm?url` (Vite core's own), making pnpm-strict transitive tooling available, a Preact measurement path, Solid/Svelte rendering.
