...
5. **Scope is the measured component's module graph.** An identifier is supplied only to a module
   inside that graph, never to `node_modules` and never to a module that already imports or declares
   it. Component registration is additive and never shadows an explicit import.
...
- A new module, `src/project/generated-declarations.ts`, owns the parsing. It is a text parser over
  a declaration file's `typeof import('<module>')` shapes; it does not use the TypeScript program,
  because these files are read whether or not the component's program includes them.
- The harness's Vue entry gains a registration block before `createApp`
  (`src/harness/entry.ts:308`), and the harness's plugin list gains one transform.
- A Vue project whose maps are stale — regenerated by a build the developer has not run — resolves to
  what the map says, not to what the source says. This is the same staleness the project's own editor
  tooling lives with, and it is disclosed by decision item 6.
- The maps' shapes are set by two third-party plugins and by Nuxt. A shape change breaks parsing, so
  the parser fails soft: an unparseable line is skipped and counted, never fatal.
