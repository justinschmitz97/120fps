...
// Styling generated live in the browser, so no static stylesheet was
...
// find.
// The list is what the recogniser can name, not what exists: a package whose
// only styling is `makeStyles` reads as "no stylesheet found" whenever the
// engine it uses (Griffel, for Fluent v9) is missing from the list.
...
// The bindings a runtime styling engine exports. An import of one of
...
// disclosed in weaker wording (report/terminal.ts, formatStylesheetsLine).
...
// Styling generated by a build step the harness does not run. Recognized
...
// `importedPackages` is the same `externalDeps` list the harness already
// builds from the measured component's (and wrapper's) resolved imports.
// Membership there, not manifest/resolution-chain availability, is what
// earns this warning: package/resolution-chain availability alone would let
// a workspace member that merely declares @linaria/core get the warning on
// every component in it, including one whose own import graph never touches
// it.
...
// Tailwind 3 has no Vite plugin of its own; it enters
...
// Names what was looked for, where, and the directory this run was
...
// The member's pipeline rebuilt with one difference -- its Tailwind 3
...
// Tailwind 3 resolves a relative `content` glob against
...
// The Tailwind plugin generates utility CSS for the classes the measured
