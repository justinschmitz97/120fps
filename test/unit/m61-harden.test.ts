...
// H4: cap larger than the cell count.
describe("H4: matrix cap larger than the cell count", () => {
  it("returns identity selection unchanged", () => {
    const axes = [{ propName: "a", values: [false, true] }];
    const combos: PropCombination[] = [{ a: false }, { a: true }];
    expect(selectMatrixCombos(combos, axes, 100)).toEqual([0, 1]);
  });
});

// H5: a baseline file saved before scaleProbe existed loads unaffected.
describe("H5: baseline saved pre-M61 loaded post-M61", () => {
...
describe("H10: scale-probe label composes with M59/M64 formatting", () => {
