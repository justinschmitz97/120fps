...
describe("H2: duplicate stylesheet paths", () => {
  ...
  it("collapses a triple repeat", () => {
    const a = writeCss("styles/a.css");
    expect(resolveCssFiles({ cssFiles: [a, a, a] }, tmpDir).files).toEqual([a]);
  });
});
...
describe("H6: several detection candidates", () => {
  ...
  it("falls through to the next candidate when the first is a directory", () => {
    fs.mkdirSync(path.join(tmpDir, "app", "globals.css"), { recursive: true });
    const second = writeCss("app/global.css");
    expect(detectGlobalCss(tmpDir)).toBe(second);
  });
...
describe("H19: injection with auto-scale rendering", () => {
  ...
  it("emits exactly one import line per stylesheet", () => {
    const entry = generateEntry({
      componentRelative: "src/Card.tsx",
      componentName: "Card",
      isDefaultExport: true,
      hasScale: false,
      cssImports: ["/a.css", "/b.css"],
    });
    expect(entry.split('import "/a.css";').length - 1).toBe(1);
    expect(entry.split('import "/b.css";').length - 1).toBe(1);
  });
});
