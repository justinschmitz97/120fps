...
describe("M63 §1 — superlinear promotion requires a magnitude and a fit margin", () => {
  ...
  it("keeps M53's raw-y ranking among admitted candidates", () => {
...
describe("M63 §2 — classification is stable under noise", () => {
...
describe("M63 §3 — an unactivated --curve is announced", () => {
...
describe("M63 §4 — curve FAIL names the budget and the crossing point", () => {
...
describe("M63 §5 — growth column and superlinear hint share one classification", () => {
  ...
  it("prints the class of every curve the hint reads", () => {
    const built = buildPassingCurve();
    built.rerenderCurve = { slope: 1, intercept: 0, r2: 0.99, growthClass: "exponential" };
    const report = makeCurveModeReport(built, true);
    const out = formatTable(report);

    expect(hintsForReport(report)).toContain("superlinearGrowth");
    // The hint fires off the rerender curve; the screen must name that class.
    expect(out).toContain("Growth: mount linear, rerender exponential");
    expect(out).toContain("superlinear");
  });

  it("prints no superlinear hint when neither printed class is superlinear", () => {
    const report = makeCurveModeReport(buildPassingCurve(), true);
    const out = formatTable(report);
    expect(out).toContain("Growth: mount linear, rerender linear");
    expect(hintsForReport(report)).not.toContain("superlinearGrowth");
  });
});
