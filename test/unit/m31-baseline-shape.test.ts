...
describe("m31 C3 — baselines record which measurement semantics produced them", () => {
  ...
  it("treats a pre-M31 baseline as incompatible rather than comparable", () => {
    const older = fingerprint({ metrics: undefined });
    expect(classifyEnv(older, fingerprint())).toBe("incompatible");
  });

  it("leaves M29 field-shape versioning alone", () => {
