...
describe("H3: exotic types without a name", () => {
  ...
  it("returns the host tag for a string type", () => {
    expect(resolveTypeName("span", 0)).toBe("span");
  });
});
...
describe("H7: report.mode and baseline fingerprint compatibility", () => {
  it("assigns to the fingerprint's mode field", () => {
    const modes: ReportMode[] = ["combo", "curve", "matrix", "isolation"];
    const asFingerprintModes: ("combo" | "curve" | "matrix" | "isolation")[] = modes;
    expect(asFingerprintModes).toEqual(modes);
  });
...
describe("H12: baseline clause conditions", () => {
  it("stays off when --check found no entry (no baseline field)", () => {
